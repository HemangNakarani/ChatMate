...
public class SpinMenu extends FrameLayout {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            try {
                ...
                if (Math.abs(e1.getY() - e2.getY()) > SWIPE_MAX_OFF_PATH && Math.abs(velocityY) > SWIPE_THRESHOLD_VELOCITY)
                    Toast.makeText(getContext(), "Downward Swipe", Toast.LENGTH_SHORT).show();
                else if (Math.abs(e2.getY() - e1.getY()) > SWIPE_MAX_OFF_PATH && Math.abs(velocityY) > SWIPE_THRESHOLD_VELOCITY)
                    Toast.makeText(getContext(), "Upward Swipe", Toast.LENGTH_SHORT).show();
                    // right to left swipe*/
    ...
    protected void onFinishInflate() {
        ...
        @SuppressLint("ResourceType") @IdRes final int smLayoutId = 0x6F060505;
